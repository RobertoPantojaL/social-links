...
app.get("/api/profile", auth, async (req, res) => {
  try {
    const [rows] = await promisePool.query(
      "SELECT id, username, email, theme, profile_image FROM users WHERE id = ?",
      [req.userId]
    );
    if (rows.length === 0) {
...
app.put("/api/profile-image", auth, upload.single("profile_image"), async (req, res) => {
  try {
    ...
    await promisePool.query("UPDATE users SET profile_image = ? WHERE id = ?", [
      imageUrl,
      req.userId,
    ]);
...
app.put("/api/theme", auth, async (req, res) => {
  try {
    ...
    await promisePool.query("UPDATE users SET theme = ? WHERE id = ?", [
...
app.get("/u/:username", async (req, res) => {
  try {
    const [rows] = await promisePool.query(
      "SELECT id, username, theme, profile_image FROM users WHERE username = ?",
      [req.params.username]
    );
    if (rows.length === 0) {
      return res.status(404).json({ message: "Usuario no encontrado" });
    ...
    const user = rows[0];
    const [links] = await promisePool.query("SELECT title, url FROM links WHERE user_id = ?", [
      user.id,
    ]);
    res.json({ ...user, links });
  } catch (error) {
